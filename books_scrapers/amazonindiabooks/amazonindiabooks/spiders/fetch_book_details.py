...
class FetchBookDetailsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        #yield scrapy.Request("https://www.amazon.com/Book-Why-Science-Cause-Effect/dp/046509760X")
        urls_batch = self.get_urls()
        urls = next(urls_batch)
        # base = "https://api.proxycrawl.com/scraper?token=XSYgFM2lbcakLWyfo806Hw&url="
        for i in range(self.batch_size):
            yield scrapy.Request(urls.pop(0), dont_filter=True)
    ...
    def parse(self, response):
        ...
        try:
            ...
            book_image_url = soup.find_all('img', {'class':'a-dynamic-image'})[0].attrs['data-a-dynamic-image'].partition(':[')[0][2:-1]
